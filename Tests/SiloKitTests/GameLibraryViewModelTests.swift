...
struct GameLibraryViewModelTests {
    ...
    func learnedHintDroppedWhenRuntimeGone() async throws {
        let tmp = try TempDir(); defer { tmp.cleanup() }
        // Wine configured but NO DXMT installed, while the game's hint points at DXMT — i.e. the user
        // removed that runtime in Settings after it had been learned. (Upstream d2addb8, adapted: their
        // version used DXVK, which this fork doesn't carry.)
        let (vm, fake, paths) = make(tmp)
        try installSteam(paths)
        _ = try await ConfigStore(paths: paths).saveGame(
            GameConfig(appID: 220, graphics: .auto, learnedBackend: .dxmt))
        ...
        // It must NOT dead-end on "DXMT isn't installed": the hint is dropped and Automatic falls back to
