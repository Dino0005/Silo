...
public struct GPTKImporter: Sendable {
    ...
    /// A runtime name is a flat label, never a path — and here it's *derived from a filename the user
    ...
    /// reduces to `".."`. (From upstream 27dde3f.)
    ...
    public func importGPTK(
        ...
        do {
            ...
            // Validate the COPIED tree before publishing. `redist/lib` existing on the volume only proves
            // the image had that folder — it doesn't prove this is Apple's GPTK. Without this a wrong .dmg
            // was copied, published, and reported as "Imported <name>." while `installed()` (which checks
            // the real signature) found nothing, leaving the onboarding step stuck at not-Done with no
            // reason given. Check the same signature here so the failure is named where it happens.
